...
async def create_member_working_hours(
    ...
    if existing:
        raise ConflictException("Juz istnieje wpis dla tego pracownika w tym dniu. Uzyj PUT do aktualizacji.")
...
async def delete_member_working_hours(exception_id: UUID, db: AsyncSession = Depends(get_db), admin: bool = Depends(get_current_admin)):
    repo = MemberWorkingHoursRepository(db)
    await repo.delete(exception_id)
    return None

@router.get("/member/{team_member_id}/date/{work_date}", response_model=MemberWorkingHoursResponse)
async def get_member_working_hours_for_date(team_member_id: UUID, work_date: date, db: AsyncSession = Depends(get_db)):
    repo = MemberWorkingHoursRepository(db)
    exception = await repo.get_by_member_and_date(team_member_id, work_date)
    
    if not exception:
        raise NotFoundException("Brak wyjatkow dla tego dnia")
    return MemberWorkingHoursResponse(
        id=exception.id,
        team_member_id=exception.team_member_id,
        work_date=exception.work_date,
