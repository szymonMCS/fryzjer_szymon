...
from src.core.exceptions import ValidationException, ServiceUnavailableException
...
def _sanitize_query(query: str) -> str:
    ...
    for phrase in FORBIDDEN_PHRASES:
        if phrase in lower_query:
            raise ValidationException("Nieprawidłowe zapytanie")
    return query[:2000]
...
async def search_knowledge(request: RAGSearchRequest, knowledge_service: KnowledgeRAGService = Depends(get_knowledge_service)):
    if not settings.OPENAI_API_KEY:
        raise ServiceUnavailableException("Brak konfiguracji API OpenAI")
